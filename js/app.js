...
// ─── SETTINGS (Eid Theme etc) ─────────────────────────────────────
async function getSettings() {
  try {
    const snap = await fdb.collection('settings').doc('global').get();
    return snap.exists ? snap.data() : {};
  } catch { return {}; }
}
async function saveSettings(data) {
  await fdb.collection('settings').doc('global').set(data, { merge: true });
}
...
async function getUserByReferralCode(code) {
  if (!code) return null;
  try {
    const snap = await fdb.collection('users').where('referralCode','==',code).limit(1).get();
    if (snap.empty) return null;
    return { id: snap.docs[0].id, ...snap.docs[0].data() };
  } catch { return null; }
}

// ─── CREATE PENDING EARNING FOR RESELLER ─────────────────────────
async function createPendingEarning({ userId, orderId, catalogId, catalogTitle, amount, currency }) {
  await fdb.collection('earnings').add({
    userId, orderId, catalogId, catalogTitle,
    amount, currency: currency || 'PKR',
    status: 'pending',
    createdAt: firebase.firestore.FieldValue.serverTimestamp(),
  });
...
// ─── CONFIRM EARNINGS WHEN ORDER DELIVERED ───────────────────────
async function confirmEarningsForOrder(orderId) {
  try {
    const snap = await fdb.collection('earnings')
      .where('orderId','==',orderId)
      .where('status','==','pending').get();
    const batch = fdb.batch();
    snap.docs.forEach(d => {
      batch.update(d.ref, { status: 'confirmed', confirmedAt: firebase.firestore.FieldValue.serverTimestamp() });
    ...
    await batch.commit();
  } catch(e) { console.warn('confirmEarnings error:', e); }
}
...
function updateNavUI() {
  ...
  // Always show bottom nav — guests can browse & order without login
...
async function submitOrder(catalogId, title, price, currency, profit, type) {
  ...
  // Get referral reseller from sessionStorage
  const savedRef  = sessionStorage.getItem('mich_ref_code') || '';
  const savedRefId= sessionStorage.getItem('mich_ref_uid')  || '';
  let resellerId  = savedRefId || (currentUser?.uid || null);
  ...
  try {
    ...
    // Create pending earning for the reseller if profit > 0 and reseller is known
    if (resellerId && profit > 0 && resellerId !== (currentUser?.uid)) {
      await createPendingEarning({
        userId: resellerId, orderId, catalogId,
        catalogTitle: title, amount: profit, currency,
      }).catch(()=>{});
    } else if (resellerId && profit > 0 && !savedRefId && currentUser?.uid && ['reseller','marketer','admin'].includes(userProfile?.role)) {
      // Order placed by the reseller themselves for a client
      await createPendingEarning({
        userId: resellerId, orderId, catalogId,
        catalogTitle: title, amount: profit, currency,
      }).catch(()=>{});
...
async function renderEarnings() {
  if (!currentUser) {
    setContent(`<div class="page"><div class="empty" style="text-align:center;padding:60px"><div class="empty-icon">💰</div><div class="empty-title">Login Required</div><div class="empty-text" style="margin-bottom:20px">Login or create a free account to track your earnings</div><button class="btn-neon" onclick="navigate('auth')">Login / Sign Up 🚀</button></div></div>`);
    return;
  }
...
async function renderOrders() {
  if (!currentUser) {
    setContent(`<div class="page"><div class="empty" style="text-align:center;padding:60px"><div class="empty-icon">🛒</div><div class="empty-title">Login to View Orders</div><div class="empty-text" style="margin-bottom:20px">Sign in to track your orders and manage your account</div><button class="btn-neon" onclick="navigate('auth')">Login / Sign Up 🚀</button></div></div>`);
    return;
  }
...
async function renderClients() {
  if (!currentUser) {
    setContent(`<div class="page"><div class="empty" style="text-align:center;padding:60px"><div class="empty-icon">👥</div><div class="empty-title">Login Required</div><div class="empty-text" style="margin-bottom:20px">Sign in to manage your clients</div><button class="btn-neon" onclick="navigate('auth')">Login / Sign Up 🚀</button></div></div>`);
    return;
  }
  setContent(`<div class="page"><div style="text-align:center;padding:60px"><div style="font-size:2rem">⏳</div></div></div>`);
...
async function renderProfile() {
  if (!currentUser) {
    setContent(`<div class="page"><div class="empty" style="text-align:center;padding:60px"><div class="empty-icon">👤</div><div class="empty-title">Login Required</div><div class="empty-text" style="margin-bottom:20px">Sign in to view and edit your profile</div><button class="btn-neon" onclick="navigate('auth')">Login / Sign Up 🚀</button></div></div>`);
    return;
  }
  const p = userProfile || {};
    ...
    <div style="display:flex;flex-direction:column;gap:16px">
      ...
      <div class="card">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:16px">
          <div>
            <div style="font-weight:700;font-size:1rem">🌙 Eid Theme & Animation</div>
            <div style="font-size:0.8rem;color:var(--text3);margin-top:4px">Show floating Eid animation, banner & countdown on home page. May 2026 Eid ul Fitr.</div>
          </div>
          <label class="toggle-switch">
            <input type="checkbox" id="toggle-eid" ${settings.eidThemeEnabled ? 'checked' : ''} onchange="toggleEidSetting(this.checked)" />
      ...
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;font-size:0.85rem">
          <div style="background:var(--bg3);border-radius:10px;padding:10px;text-align:center">
            <div style="font-size:1.3rem;font-weight:800;color:var(--blue)">${(window._adminOrders||[]).length}</div>
            <div style="color:var(--text3)">Total Orders</div>
          </div>
          <div style="background:var(--bg3);border-radius:10px;padding:10px;text-align:center">
            <div style="font-size:1.3rem;font-weight:800;color:var(--green)">${(window._adminCats||[]).length}</div>
            <div style="color:var(--text3)">Products</div>
          </div>
          <div style="background:var(--bg3);border-radius:10px;padding:10px;text-align:center">
            <div style="font-size:1.3rem;font-weight:800;color:var(--purple)">${(window._adminUsers||[]).length}</div>
            <div style="color:var(--text3)">Users</div>
          </div>
          <div style="background:var(--bg3);border-radius:10px;padding:10px;text-align:center">
            <div style="font-size:1.3rem;font-weight:800;color:var(--yellow)">${(window._adminWDs||[]).length}</div>
            <div style="color:var(--text3)">Withdrawals</div>
          </div>
        </div>
      </div>
    </div>`;
...
async function toggleEidSetting(enabled) {
  await saveSettings({ eidThemeEnabled: enabled });
  applyEidTheme(enabled);
  showToast(enabled ? '🌙 Eid theme enabled!' : 'Eid theme disabled', 'success');
}
...
async function setOrderStatus(id, status) {
  try {
    ...
    // When order is delivered, confirm the pending earnings for reseller
    if (status === 'delivered') {
      await confirmEarningsForOrder(id);
    }
...
// ─── EID THEME STATE ────────────────────────────────────────────
let _eidThemeEnabled = false;

function applyEidTheme(enabled) {
  _eidThemeEnabled = enabled;
  document.body.classList.toggle('eid-theme', enabled);
  const banner = document.getElementById('eid-banner');
  if (banner) banner.style.display = enabled ? '' : 'none';
}

async function loadEidTheme() {
  const settings = await getSettings();
  applyEidTheme(!!settings.eidThemeEnabled);
}

// ─── EID UL FITR COUNTDOWN (May 2026) ───────────────────────────
function getEidCountdown() {
  const eidDate = new Date('2026-05-27T00:00:00');
  const now     = new Date();
  const diff    = eidDate - now;
  if (diff <= 0) return null; // Eid is here or passed
...
function renderEidHeroCard() {
  if (!_eidThemeEnabled) return '';
  const cd = getEidCountdown();
  if (!cd) {
    // Eid is here!
    return `
      <div class="eid-hero-card section">
        <div class="eid-float-anim"></div>
        <div class="eid-sheep-anim">🌙</div>
        <div class="eid-hero-title">🌙 عید مبارک! 🌙</div>
        <div class="eid-hero-sub">Eid Special Sale — May 2026 🎉</div>
  ...
  return `
    <div class="eid-hero-card section">
      <div class="eid-float-anim"></div>
      <div class="eid-sheep-anim">🌙</div>
      <div class="eid-hero-title">🌙 Eid Sale — May 2026 🌙</div>
      <div class="eid-hero-sub">Special discounts coming in May! 🎁</div>
...
async function renderCatalogDetailV3(params={}) {
  ...
  // Store images globally for lightbox (avoids JSON injection in onclick)
  ...
  setContent(`
    <div class="page">
      ...
      <div class="detail-grid">
        ...
        <div class="img-gallery">
          <div class="img-main" style="cursor:zoom-in" onclick="openLightbox(window._currentProductImages, 0)">
            ${images[0]
          ...
          ${images.length > 1 ? `
            <div class="img-thumbs">
              ${images.map((img, i) => `
                <div class="img-thumb ${i===0?'active':''}" onclick="switchImgV3('${img}',this,${i})">
                  <img src="${img}" alt="" />
                </div>`).join('')}
            </div>` : ''}
        </div>
...
function switchImgV3(src, el, idx) {
  const main = document.getElementById('main-img');
  if (main) main.src = src;
  document.querySelectorAll('.img-thumb').forEach(t => t.classList.remove('active'));
  el.classList.add('active');
  // Update lightbox state using global images array
  _lightboxImages = window._currentProductImages || [];
  _lightboxIndex  = idx;
}
...
document.addEventListener('DOMContentLoaded', () => {
  // Check for share/ref params in URL
  const urlParams = new URLSearchParams(window.location.search);
  const shareId   = urlParams.get('share');
  const refCode   = urlParams.get('ref');

  // Save referral code to sessionStorage so it persists across order flow
  if (refCode) {
    sessionStorage.setItem('mich_ref_code', refCode);
    // Async: resolve uid from referral code and cache it
    getUserByReferralCode(refCode).then(u => {
      if (u) sessionStorage.setItem('mich_ref_uid', u.id);
    }).catch(() => {});
  }
  ...
  fauth.onAuthStateChanged(async (firebaseUser) => {
    ...
    updateNavUI();
    // Load Eid theme from Firestore settings
    loadEidTheme().catch(() => {});
